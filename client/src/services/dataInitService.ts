...
export const initStories = async (): Promise<number> => {
  ...
  // React 严格模式和多标签页可能同时触发初始化；bulkPut 保证重复执行仍幂等。
  await db.stories.bulkPut(allStories);
...
export const initDictionary = async (): Promise<number> => {
  ...
  await db.dictionary.bulkPut(l1Dictionary as DictionaryEntry[]);
...
export const initMapData = async (): Promise<void> => {
  ...
  // 添加 L1 区域和节点
  await db.mapRegions.put(l1Region);
  const l1Nodes = generateL1Nodes();
  await db.mapNodes.bulkPut(l1Nodes);

  // 添加 L2 区域和节点
  await db.mapRegions.put(l2ValleyMapRegion);
  await db.mapNodes.bulkPut(l2ValleyMapNodes);
};
